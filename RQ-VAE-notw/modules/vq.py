...
class VectorQuantizer(nn.Module):
    ...
    def constrained_km(self, data, n_clusters=10):
        ...
        size_min = min(len(data) // (n_clusters * 2), 400) # 50 for the very first time, 10 the latter
...
#         return x_q    sususususususussu


    def vq_init(self, x, use_sk=True):
        latent = x.reshape(-1, x.shape[-1])
        ...
        if _distance_flag == 'distance':
            d = torch.sum(latent**2, dim=1, keepdim=True) + \
                torch.sum(self.embedding.weight[:, :x.shape[-1]]**2, dim=1, keepdim=True).t() - \
                2 * torch.matmul(latent, self.embedding.weight[:, :x.shape[-1]].t())
        else:
            d = latent @ self.embedding.weight[:, :x.shape[-1]].t()
        ...
        x_q = self.embedding(indices)[:, :x.shape[-1]].reshape(x.shape)
        return x_q

    
    # def forward(self,  x, label, idx, use_sk=True):
    def forward(self,  x, idx, input_dim):
        # Flatten input
        latent = x.view(-1, input_dim)
        ...
        if _distance_flag == 'distance':
            d = torch.sum(latent**2, dim=1, keepdim=True) + \
                torch.sum(self.embedding.weight[:, :input_dim]**2, dim=1, keepdim=True).t()- \
                2 * torch.matmul(latent, self.embedding.weight[:, :input_dim].t())
        else:    
        # Calculate Cosine Similarity 
            d = latent@self.embedding.weight.t()
        ...
        x_q = self.embedding(indices)[:, :input_dim].reshape(x.shape)
