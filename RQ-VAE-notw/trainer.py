...
class Trainer(object):
    ...
    def _train_epoch(self, train_data, epoch_idx):
        ...
        for batch_idx, data in enumerate(iter_data):
            ...
            out, rq_loss, indices, dense_out, recon_in_loss = self.model(data, self.labels)

            # loss, cf_loss, loss_recon, quant_loss = self.model.compute_loss(out, rq_loss, emb_idx, dense_out, xs=data)
            loss, loss_recon, loss_recon_in, quant_loss = self.model.compute_loss(out, rq_loss, recon_in_loss, emb_idx, dense_out, xs=data)
            self.logger.info(f"batch: {batch_idx}:  loss: {loss}, loss_recon: {loss_recon}, quant_loss: {quant_loss}, loss_recon_in: {loss_recon_in}")
            ...
            total_recon_in_loss += loss_recon_in.item()
        ...
        return total_loss, total_recon_loss, total_recon_in_loss, quant_loss.item()
...
#             indices = indices.view(-1,indices.shape[-1]).cpu().numpy()
            indices = self.model.get_indices(data, self.labels)
            indices = indices.reshape(-1, indices.shape[-1]).cpu().numpy()
    ...
    def _generate_train_loss_output(self, epoch_idx, s_time, e_time, loss, recon_loss, recon_in_loss, quant_loss):
        train_loss_output = (
        ...
        train_loss_output +=", "
        train_loss_output += set_color("quant loss", "blue") + ": %.4f" % quant_loss
        train_loss_output +=", "
        train_loss_output += set_color("reconstruction in loss", "blue") + ": %.4f" % recon_in_loss
        # train_loss_output +=", "
    ...
    def fit(self, data):
        ...
        for epoch_idx in range(self.epochs):
            ...
            train_loss, train_recon_loss, total_recon_in_loss, quant_loss = self._train_epoch(data, epoch_idx)
            ...
            # )
            train_loss_output = self._generate_train_loss_output(
                epoch_idx, training_start_time, training_end_time, train_loss, train_recon_loss, total_recon_in_loss, quant_loss
            )
